...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				processedRequest = checkMultipart(request); // 1 文件上传预处理
				multipartRequestParsed = (processedRequest != request);

				// Determine handler for the current request.
				mappedHandler = getHandler(processedRequest); // 2 通过HandlerMapping 拿到可以处理请求的Handler
				...
				HandlerAdapter ha = getHandlerAdapter(mappedHandler.getHandler()); // 3 确定合适的HandlerAdapter
				...
				if (!mappedHandler.applyPreHandle(processedRequest, response)) { // 拦截器的前置处理
					return;
				}

				// Actually invoke the handler.
				mv = ha.handle(processedRequest, response, mappedHandler.getHandler()); // 4 HandlerAdapter通过反射，调用目标方法

				if (asyncManager.isConcurrentHandlingStarted()) {
					return;
				}

				applyDefaultViewName(processedRequest, mv); // 应用默认的视图名字
				mappedHandler.applyPostHandle(processedRequest, response, mv); // 拦截器的后置处理
			}
			catch (Exception ex) {
